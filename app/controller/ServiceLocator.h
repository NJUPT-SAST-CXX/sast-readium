...
/**
 * @brief ServiceLocator - Provides dependency injection through service location
 * 
...
public:
    ...
    std::shared_ptr<Interface> getSharedService() {
        ...
        if (m_sharedServices.contains(typeName)) {
            return std::static_pointer_cast<Interface>(m_sharedServices[typeName]);
        }
        return nullptr;
    }
...
private:
    ...
    void registerServiceFactory(const QString& typeName, ServiceFactory factory);
...
public:
    ...
    void provideService(Interface* service) {
        ...
        m_providedServices.append(QString::fromStdString(typeid(Interface).name()));
    }
...
public:
    ...
    static void injectService(Object* object, void (Object::*setter)(Service*)) {

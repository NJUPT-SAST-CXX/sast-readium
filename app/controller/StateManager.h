...
private:
    ...
    // Helper methods
    QJsonValue getValueByPath(const QJsonObject& obj, const QStringList& path) const;
    QJsonObject setValueByPath(const QJsonObject& obj, const QStringList& path, const QJsonValue& value) const;
};
...
class StateChange {
public:
    StateChange(const State& oldState, const State& newState, const QString& reason = QString());
...
public:
    ...
    // State mutations
    void set(const QString& path, const QVariant& value, const QString& reason = QString());
    ...
    void reset(const State& newState = State(), const QString& reason = "Reset");
    
    // Observers
    using StateObserver = std::function<void(const StateChange&)>;
    void subscribe(const QString& path, QObject* subscriber, StateObserver observer);
...
signals:
    ...
    void stateChanged(const QString& path, const QVariant& oldValue, const QVariant& newValue);
...
public:
    explicit StateStore(const State& initialState = State(), QObject* parent = nullptr);
...
public:
    ...
    explicit StateSelector(SelectorFunc selector)
        : m_selector(selector) {}
...
#define STATE_SUBSCRIBE(path, handler) StateManager::instance().subscribe(path, this, handler)

...
public:
    ...
    bool isIntelligentPrefetchEnabled() const {
        return m_intelligentPrefetchEnabled;
    }
    ...
    bool isPredictiveLoadingEnabled() const {
...
private:
    ...
    struct AccessPattern {
        ...
        AccessPattern() { sessionTimer.start(); }
    };
    ...
    void addToPrefetchQueue(int pageNumber, PrefetchStrategy strategy,
                            int priority = 0);
    void predictNextPages(const QList<int>& recentAccesses,
    ...
    mutable QHash<int, ThumbnailItem>
        m_thumbnails;  // Fallback for compatibility
    mutable QMultiHash<int, int>
        m_accessFrequencyIndex;        // frequency -> pageNumber mapping

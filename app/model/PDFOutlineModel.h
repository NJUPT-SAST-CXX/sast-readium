...
struct PDFOutlineNode : public std::enable_shared_from_this<PDFOutlineNode> {
    ...
    // 查找指定标题的子节点
    std::shared_ptr<PDFOutlineNode> findChildByTitle(
        const QString& title, bool caseSensitive = false) const;
...
public:
    ...
    // 根据标题搜索目录节点（支持部分匹配）
    QList<std::shared_ptr<PDFOutlineNode>> searchByTitle(
        const QString& title, bool caseSensitive = false) const;
...
private:
    ...
    void searchByTitleRecursive(
        const QList<std::shared_ptr<PDFOutlineNode>>& nodes,
        const QString& title, bool caseSensitive,
        QList<std::shared_ptr<PDFOutlineNode>>& result) const;

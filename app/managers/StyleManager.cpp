...
public:
    ...
    QString getQssStyleSheet() const;
...
QString StyleManager::getApplicationStyleSheet() const {
    ...
    QString stylesheet = pImpl->getQssStyleSheet();

    // Append minimal programmatic styles only for components requiring dynamic color injection
    // QSS files handle the majority of styling - these are only for elements that need
    // runtime theme color access that QSS cannot provide
...
QString StyleManager::createButtonStyle() const {
    ...
    if (pImpl->m_currentTheme == Theme::Dark) {
        disabledBg = disabledBg.darker(135);
    } else {
        disabledBg = disabledBg.lighter(104);
    }

    QString transition = getTransitionStyle("background-color, border-color, box-shadow",
                                            animationFast());
...
QColor StyleManager::surfaceAltColor() const { return pImpl->m_surfaceAltColor; }
QColor StyleManager::elevatedSurfaceColor() const {
    return pImpl->m_elevatedSurfaceColor;
}
...
QColor StyleManager::mutedBorderColor() const {
    return pImpl->m_mutedBorderColor;
}
...
QColor StyleManager::focusColor() const { return pImpl->m_focusColor; }
QColor StyleManager::overlayColor() const { return pImpl->m_overlayColor; }
...
QString StyleManager::createToggleButtonStyle() const {
    ...
    QString transition = getTransitionStyle("background-color, color, border-color",
                                            animationFast());
...
void StyleManagerImpl::loadQssFiles() {
    Logger::instance().info("[managers] Loading QSS theme files from resources...");
...
QColor StyleManager::successColor() const {
    if (pImpl->m_currentTheme == Theme::Light) {

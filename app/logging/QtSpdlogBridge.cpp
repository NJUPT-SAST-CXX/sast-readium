...
Logger::LogLevel QtSpdlogBridge::Implementation::qtMsgTypeToLogLevel(
...
QString QtSpdlogBridge::Implementation::formatQtMessage(
...
SpdlogQDebug& SpdlogQDebug::operator<<(const QRect& rect) {
    d->stream << "QRect(" << rect.x() << "," << rect.y() << " " << rect.width()
              << "x" << rect.height() << ")";
    return *this;
}

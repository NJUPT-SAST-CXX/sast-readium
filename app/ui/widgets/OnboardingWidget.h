 ...
 * OnboardingWidget
 * 
 * An overlay widget that provides interactive guided tours for first-time users.
 * Features tooltips, highlights, and step-by-step navigation through app features.
 */
class OnboardingWidget : public QWidget {
    Q_OBJECT
    Q_PROPERTY(qreal overlayOpacity READ overlayOpacity WRITE setOverlayOpacity)
    Q_PROPERTY(QPoint tooltipPosition READ tooltipPosition WRITE setTooltipPosition)
...
public:
    ...
    void showTooltip(const QString& title, const QString& description, 
                    const QPoint& position, Qt::Alignment alignment = Qt::AlignCenter);
    void hideTooltip();

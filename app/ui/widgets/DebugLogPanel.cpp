...
void DebugLogPanel::setupFilterControls() {
    ...
    m_logLevelFilter->addItem(tr("Info+"),
                              static_cast<int>(Logger::LogLevel::Info));
    m_logLevelFilter->addItem(tr("Warning+"),
                              static_cast<int>(Logger::LogLevel::Warning));
    m_logLevelFilter->addItem(tr("Error+"),
    ...
    if (!searchTerm.isEmpty()) {
        ...
        if (m_config.regexSearch) {
            ...
            if (match.hasMatch() && match.capturedStart() == position) {
                cursor.setPosition(position);
                cursor.setPosition(position + match.capturedLength(),
                                   QTextCursor::KeepAnchor);
            }
        } else {
            // For plain text search
            cursor.setPosition(position);
            cursor.setPosition(position + searchTerm.length(),
                               QTextCursor::KeepAnchor);
        }
    }
...
void DebugLogPanel::updateStatisticsDisplay() {
    ...
    m_messagesPerSecLabel->setText(
        tr("Messages/sec: %1").arg(stats.messagesPerSecond, 0, 'f', 2));
    ...
    for (QLabel* label : labels) {
        if (label->text() == "Level:" || label->text().contains("Level")) {
            label->setText(tr("Level:"));
                   ...
                   label->text().contains("Category")) {
            label->setText(tr("Category:"));
        } else if (label->text() == "Search:" ||
                   label->text().contains("Search")) {
            label->setText(tr("Search:"));
        } else if (label->text() == "Memory:" ||
                   label->text().contains("Memory")) {

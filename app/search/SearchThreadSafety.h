 ...
 * Provides thread-safe access patterns, atomic operations, and synchronization primitives
 */
...
public:
    ...
    {
    public:
        explicit SharedData(const T& initialValue = T()) : m_data(initialValue) {}
        
        // Read access (multiple readers allowed)
        class ReadAccess
        {
        public:
            explicit ReadAccess(SharedData* parent) : m_locker(&parent->m_lock), m_data(&parent->m_data) {}
        ...
        {
        public:
            explicit WriteAccess(SharedData* parent) : m_locker(&parent->m_lock), m_data(&parent->m_data) {}
    ...
    private:
        ...
        void lockAll(Mutex& mutex) {
            m_lockers.emplace_back(std::make_unique<QMutexLocker<QMutex>>(&mutex));
        }
    ...
    public:
        struct ContentionStats {
            ...
            double contentionRate() const {
                return lockAttempts > 0 ? (double)lockContentions / lockAttempts : 0.0;
            }
        };
        
        static void recordLockAttempt(const QString& mutexName);
        static void recordLockContention(const QString& mutexName, qint64 waitTime);
...
#define SEARCH_HIERARCHICAL_MUTEX(name, level) \
    SearchThreadSafety::MutexHierarchy::HierarchicalMutex name(SearchThreadSafety::MutexHierarchy::level)

#define SEARCH_MULTI_LOCK(...) \
    SearchThreadSafety::MultiLockGuard _guard(__VA_ARGS__)

#define SEARCH_READ_LOCK(sharedData) \
    auto _readLock = sharedData.read()

#define SEARCH_WRITE_LOCK(sharedData) \
    auto _writeLock = sharedData.write()

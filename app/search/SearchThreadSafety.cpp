...
thread_local SearchThreadSafety::MutexHierarchy::Level SearchThreadSafety::MutexHierarchy::s_currentLevel = 
    SearchThreadSafety::MutexHierarchy::MetricsLevel;
QHash<QThread*, SearchThreadSafety::MutexHierarchy::Level> SearchThreadSafety::MutexHierarchy::s_threadLevels;
QMutex SearchThreadSafety::MutexHierarchy::s_hierarchyMutex;

QHash<QString, SearchThreadSafety::ContentionMonitor::ContentionStats> SearchThreadSafety::ContentionMonitor::s_stats;
...
void SearchThreadSafety::MutexHierarchy::HierarchicalMutex::checkHierarchy()
{
    ...
    if (s_threadLevels.contains(currentThread)) {
        ...
        if (m_level >= currentLevel) {
            qWarning() << "Potential deadlock detected: Attempting to acquire mutex at level" 
...
void SearchThreadSafety::MutexHierarchy::HierarchicalMutex::setCurrentLevel(Level level)
{
    QMutexLocker locker(&s_hierarchyMutex);
    s_threadLevels[QThread::currentThread()] = level;
}

void SearchThreadSafety::MutexHierarchy::HierarchicalMutex::clearCurrentLevel()
{
    QMutexLocker locker(&s_hierarchyMutex);
    QThread* currentThread = QThread::currentThread();
...
void SearchThreadSafety::MutexHierarchy::validateHierarchy()
{
    ...
    for (auto it = s_threadLevels.begin(); it != s_threadLevels.end(); ++it) {
        ...
        qDebug() << "Thread" << thread << "currently holds lock at level" << level;
    }
}

// ContentionMonitor implementation
void SearchThreadSafety::ContentionMonitor::recordLockAttempt(const QString& mutexName)
{
    QMutexLocker locker(&s_statsMutex);
    s_stats[mutexName].lockAttempts++;
}

void SearchThreadSafety::ContentionMonitor::recordLockContention(const QString& mutexName, qint64 waitTime)
{
    QMutexLocker locker(&s_statsMutex);
...
public:
    ...
    void lock() {
        ...
        if (!m_mutex.tryLock()) {
            ...
            SearchThreadSafety::ContentionMonitor::recordLockContention(m_name, waitTime);
        }
    }

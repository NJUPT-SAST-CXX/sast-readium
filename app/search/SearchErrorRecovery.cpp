...
public:
        ...
        , circuitBreakerTimer(new QTimer(q))
    {
        ...
        QObject::connect(recoveryTimer, &QTimer::timeout, q, &SearchErrorRecovery::onRecoveryTimer);
        QObject::connect(circuitBreakerTimer, &QTimer::timeout, q, &SearchErrorRecovery::onCircuitBreakerTimer);
        
        // Start monitoring timers
        recoveryTimer->start(5000); // Check every 5 seconds
        circuitBreakerTimer->start(10000); // Check circuit breakers every 10 seconds
    }
...
void SearchErrorRecovery::setRecoveryConfig(ErrorType type, const RecoveryConfig& config)
{
    QMutexLocker locker(&d->mutex);
    d->recoveryConfigs[type] = config;
}

SearchErrorRecovery::RecoveryConfig SearchErrorRecovery::getRecoveryConfig(ErrorType type) const
{
    QMutexLocker locker(&d->mutex);
...
SearchErrorRecovery::RecoveryResult SearchErrorRecovery::handleError(const std::exception& e, const ErrorContext& context)
{
...
SearchErrorRecovery::RecoveryResult SearchErrorRecovery::handleError(const QString& errorMessage, const ErrorContext& context)
{
    ErrorContext mutableContext = context;
...
SearchErrorRecovery::RecoveryResult SearchErrorRecovery::recoverFromError(const ErrorContext& context)
{
    QMutexLocker locker(&d->mutex);
    ...
    d->stats.recentErrors.append(QString("%1: %2").arg(context.operation, context.details));
    ...
    RecoveryConfig config = d->recoveryConfigs.value(context.type, RecoveryConfig());
    ...
    case Reset:
        ...
        break;
    default:
        result = RecoveryResult(false, NoRecovery, 0, "No recovery strategy configured");
        break;
    }
...
SearchErrorRecovery::RecoveryResult SearchErrorRecovery::retryOperation(std::function<bool()> operation, const ErrorContext& context)
{
    ...
    for (int attempt = 1; attempt <= config.maxRetries; ++attempt) {
        try {
            ...
            if (attempt >= config.maxRetries) {
                return RecoveryResult(false, Retry, attempt, QString("Retry failed: %1").arg(e.what()));
            }
        }
    }
    
    return RecoveryResult(false, Retry, config.maxRetries, "Maximum retries exceeded");
}

SearchErrorRecovery::RecoveryResult SearchErrorRecovery::fallbackOperation(const ErrorContext& context)
{
    QMutexLocker locker(&d->mutex);
    ...
    if (d->fallbackFunctions.contains(key)) {
        try {
            ...
            return RecoveryResult(true, Fallback, 1, "Fallback executed successfully");
            
        } catch (const std::exception& e) {
            return RecoveryResult(false, Fallback, 1, QString("Fallback failed: %1").arg(e.what()));
        }
    }
    
    return RecoveryResult(false, Fallback, 0, "No fallback function registered");
}

SearchErrorRecovery::RecoveryResult SearchErrorRecovery::degradeOperation(const ErrorContext& context)
{
...
SearchErrorRecovery::RecoveryResult SearchErrorRecovery::skipOperation(const ErrorContext& context)
{
...
SearchErrorRecovery::RecoveryResult SearchErrorRecovery::resetOperation(const ErrorContext& context)
{
...
void SearchErrorRecovery::enableCircuitBreaker(const QString& operationName, int failureThreshold, int timeoutMs)
{
    QMutexLocker locker(&d->mutex);
    
    Implementation::CircuitBreakerState& state = d->circuitBreakers[operationName];
...
bool SearchErrorRecovery::isCircuitBreakerOpen(const QString& operationName) const
{
    QMutexLocker locker(&d->mutex);
    ...
    if (state.openedAt.msecsTo(now) >= state.timeoutMs) {
        ...
        emit const_cast<SearchErrorRecovery*>(this)->circuitBreakerClosed(operationName);
        return false;
    }
...
void SearchErrorRecovery::saveOperationState(const QString& operationId, const QVariantMap& state)
{
    QMutexLocker locker(&d->mutex);
    d->operationStates[operationId] = state;
}

QVariantMap SearchErrorRecovery::restoreOperationState(const QString& operationId)
{
    QMutexLocker locker(&d->mutex);
...
void SearchErrorRecovery::registerFallback(ErrorType type, const QString& operation, FallbackFunction fallback)
{
    QMutexLocker locker(&d->mutex);
    d->fallbackFunctions[QPair<ErrorType, QString>(type, operation)] = fallback;
}

void SearchErrorRecovery::unregisterFallback(ErrorType type, const QString& operation)
{
    QMutexLocker locker(&d->mutex);
    d->fallbackFunctions.remove(QPair<ErrorType, QString>(type, operation));
}

QVariant SearchErrorRecovery::executeFallback(ErrorType type, const QString& operation, const ErrorContext& context)
{
    QMutexLocker locker(&d->mutex);
...
void SearchErrorRecovery::reportComponentHealth(const QString& component, bool healthy)
{
    QMutexLocker locker(&d->mutex);
...
QStringList SearchErrorRecovery::getUnhealthyComponents() const
{
    ...
    for (auto it = d->componentHealth.constBegin(); it != d->componentHealth.constEnd(); ++it) {
        if (!it.value()) {
...
SearchErrorRecovery::ErrorType SearchErrorRecovery::classifyError(const std::exception& e) const
{
    QString errorMsg = QString::fromStdString(e.what()).toLower();
    return classifyError(errorMsg);
}

SearchErrorRecovery::ErrorType SearchErrorRecovery::classifyError(const QString& errorMessage) const
{
    QString msg = errorMessage.toLower();

    if (msg.contains("validation") || msg.contains("invalid") || msg.contains("malformed")) {
        return ValidationError;
    }
    if (msg.contains("document") || msg.contains("pdf") || msg.contains("page") || msg.contains("load")) {
        return DocumentError;
    }
    if (msg.contains("search") || msg.contains("query") || msg.contains("pattern") || msg.contains("regex")) {
        return SearchError;
    }
    if (msg.contains("cache") || msg.contains("storage")) {
        return CacheError;
    }
    if (msg.contains("memory") || msg.contains("allocation") || msg.contains("out of")) {
        return MemoryError;
    }
    if (msg.contains("timeout") || msg.contains("time") || msg.contains("deadline")) {
        return TimeoutError;
    }
...
int SearchErrorRecovery::calculateRetryDelay(int attempt, const RecoveryConfig& config) const
{
    ...
    int exponentialDelay = static_cast<int>(baseDelay * std::pow(2, attempt - 1));

    // Add jitter (±25%)
    int jitter = exponentialDelay / 4;
    int randomJitter = (QRandomGenerator::global()->bounded(2 * jitter + 1)) - jitter;

    return qMax(baseDelay, exponentialDelay + randomJitter);
}

bool SearchErrorRecovery::shouldRetry(const ErrorContext& context, const RecoveryConfig& config) const
{
...
void SearchErrorRecovery::logRecoveryAttempt(const ErrorContext& context, RecoveryStrategy strategy) const
{
    ...
    switch (strategy) {
    case Retry: strategyName = "Retry"; break;
    case Fallback: strategyName = "Fallback"; break;
    case Degrade: strategyName = "Degrade"; break;
    case Skip: strategyName = "Skip"; break;
    case Reset: strategyName = "Reset"; break;
    default: strategyName = "Unknown"; break;
    }

    qDebug() << QString("SearchErrorRecovery: Attempting %1 for %2 in %3 (attempt %4): %5")
                .arg(strategyName, context.operation, context.component)
                .arg(context.attemptCount)
                .arg(context.details);
}

void SearchErrorRecovery::updateErrorStats(const ErrorContext& context, const RecoveryResult& result) const
{
...
void SearchErrorRecovery::onRecoveryTimer()
{
    ...
    while (it != d->operationStates.end()) {
        ...
        QDateTime timestamp = state.value("timestamp", QDateTime::currentDateTime()).toDateTime();
    ...
    QDateTime healthCutoff = QDateTime::currentDateTime().addSecs(-300); // 5 minutes
    for (auto it = d->lastHealthCheck.begin(); it != d->lastHealthCheck.end(); ++it) {
...
void SearchErrorRecovery::onCircuitBreakerTimer()
{
    ...
    for (auto it = d->circuitBreakers.begin(); it != d->circuitBreakers.end(); ++it) {
        auto& state = it.value();
...
SearchErrorScope::SearchErrorScope(SearchErrorRecovery* recovery, const SearchErrorRecovery::ErrorContext& context)
    : m_recovery(recovery), m_context(context), m_successful(false), m_committed(false)
{

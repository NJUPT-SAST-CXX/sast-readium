...
QList<SearchFeatures::FuzzyMatch> SearchFeatures::fuzzySearch(
    const QString& text, const QString& pattern, int maxDistance,
    int maxResults) {
    ...
    for (const QString& word : words) {
        ...
        if (distance <= maxDistance) {
            ...
            match.similarity = 1.0 - (static_cast<double>(distance) /
                                      qMax(word.length(), pattern.length()));

            // Extract context
            int contextStart = qMax(0, currentPosition - 50);
            int contextEnd =
                qMin(text.length(), currentPosition + word.length() + 50);
...
int SearchFeatures::calculateLevenshteinDistance(const QString& str1,
                                                 const QString& str2) {
    return FuzzySearchAlgorithms::levenshteinDistance(str1, str2);
}

double SearchFeatures::calculateSimilarity(const QString& str1,
                                           const QString& str2) {
...
QList<SearchResult> SearchFeatures::wildcardSearch(const QString& text,
    ...
    while (iterator.hasNext()) {
        ...
        int contextEnd =
            qMin(text.length(), result.textPosition + result.textLength + 50);
...
QList<SearchResult> SearchFeatures::phraseSearch(const QString& text,
    ...
    if (proximity == 0) {
        // Exact phrase search
        int position = 0;
        while ((position = text.indexOf(phrase, position,
            ...
            int contextEnd =
                qMin(text.length(), position + phrase.length() + 50);
            result.contextText =
        ...
        QStringList phraseWords =
            phrase.split(QRegularExpression("\\s+"), Qt::SkipEmptyParts);
...
QList<SearchResult> SearchFeatures::booleanSearch(const QString& text,
                                                  const QString& query,
                                                  int pageNumber) {
...
void SearchFeatures::setHighlightColors(const QColor& normalColor,
...
QList<SearchFeatures::HighlightInfo> SearchFeatures::generateHighlights(
    ...
    for (int i = 0; i < results.size(); ++i) {
        ...
        highlight.priority =
            results.size() - i;  // Higher priority for earlier results
...
void SearchFeatures::updateHighlightPriorities(
    QList<HighlightInfo>& highlights) {
    // Sort by priority (current result first, then by position)
    std::sort(
        highlights.begin(), highlights.end(),
        [](const HighlightInfo& a, const HighlightInfo& b) {
            if (a.isCurrentResult != b.isCurrentResult) {
                return a
                    .isCurrentResult;  // Current result has highest priority
            }
            return a.priority > b.priority;
        });
}

void SearchFeatures::addToHistory(const QString& query,
                                  const SearchOptions& options, int resultCount,
                                  qint64 searchTime, bool successful) {
...
QList<SearchFeatures::HistoryEntry> SearchFeatures::getSearchHistory(
...
QStringList SearchFeatures::generateSuggestions(const QString& partialQuery,
                                                int maxSuggestions) {
    QStringList suggestions =
        d->suggestionEngine->generateSuggestions(partialQuery, maxSuggestions);
    emit suggestionsReady(suggestions);
    return suggestions;
}

QStringList SearchFeatures::getQueryCompletions(const QString& prefix,
                                                int maxCompletions) {
...
QList<SearchResult> SearchFeatures::proximitySearch(
    const QString& text, const QStringList& terms,
...
QList<SearchResult> SearchFeatures::filterResults(
    const QList<SearchResult>& results, const QString& filterCriteria) {
    QList<SearchResult> filteredResults;

    QRegularExpression filterRegex(filterCriteria,
                                   QRegularExpression::CaseInsensitiveOption);
...
QList<SearchResult> SearchFeatures::sortResults(
    const QList<SearchResult>& results, SortCriteria criteria, bool ascending) {
    QList<SearchResult> sortedResults = results;

    std::sort(
        sortedResults.begin(), sortedResults.end(),
        [criteria, ascending](const SearchResult& a, const SearchResult& b) {
            bool result = false;
            ...
            switch (criteria) {
                ...
                default:
                    ...
                    result = a.textPosition <
                             b.textPosition;  // Fallback to position
                    break;
            }
...
bool SearchFeatures::importSearchHistory(const QString& filePath) {
    ...
    for (const QJsonValue& value : historyArray) {
        ...
        entry.timestamp = QDateTime::fromString(
            entryObj["timestamp"].toString(), Qt::ISODate);
...
QString SearchFeatures::exportSearchResults(const QList<SearchResult>& results,
...
int FuzzySearchAlgorithms::levenshteinDistance(const QString& str1,
                                               const QString& str2) {
    ...
    for (int i = 1; i <= len1; ++i) {
        for (int j = 1; j <= len2; ++j) {
            ...
            matrix[i][j] =
                qMin(qMin(matrix[i - 1][j] + 1, matrix[i][j - 1] + 1),
                     matrix[i - 1][j - 1] + cost);
        }
    }
...
int FuzzySearchAlgorithms::levenshteinDistanceOptimized(const QString& str1,
                                                        const QString& str2,
                                                        int maxDistance) {
    int len1 = str1.length();
    int len2 = str2.length();
    ...
    for (int i = 1; i <= len1; ++i) {
        ...
        for (int j = 1; j <= len2; ++j) {
            ...
            currRow[j] = qMin(qMin(currRow[j - 1] + 1, prevRow[j] + 1),
                              prevRow[j - 1] + cost);
...
int FuzzySearchAlgorithms::damerauLevenshteinDistance(const QString& str1,
                                                      const QString& str2) {
    int len1 = str1.length();
    int len2 = str2.length();
...
double FuzzySearchAlgorithms::jaroWinklerSimilarity(const QString& str1,
    ...
    double jaro =
        (static_cast<double>(matches) / len1 +
    ...
    for (int i = 1; i < word.length() && result.length() < 4; ++i) {
        ...
        if (ch.isLetter()) {
            ...
            if (index >= 0 && index < 26) {
                ...
                if (digit != '0' &&
                    (result.isEmpty() || result.back() != digit)) {
                    result += digit;
                }
            }
        }
...
bool FuzzySearchAlgorithms::soundexMatch(const QString& word1,
                                         const QString& word2) {
...
SearchHighlightEngine::SearchHighlightEngine() {
    ...
    defaultStyle.borderWidth = 1;
    defaultStyle.opacity = 0.7;
    defaultStyle.pattern =
        "background-color: %1; color: %2; border: %3px solid %4; opacity: %5;";
...
void SearchHighlightEngine::setHighlightStyle(const QString& styleName,
                                              const HighlightStyle& style) {
    m_styles[styleName] = style;
}

SearchHighlightEngine::HighlightStyle SearchHighlightEngine::getHighlightStyle(
...
void SearchHighlightEngine::optimizeHighlights(
    QList<SearchFeatures::HighlightInfo>& highlights) {
    // Remove duplicate highlights
    for (int i = highlights.size() - 1; i >= 0; --i) {
        for (int j = i - 1; j >= 0; --j) {
            if (highlights[i].rect == highlights[j].rect &&
                highlights[i].text == highlights[j].text) {
...
void SearchHighlightEngine::mergeOverlappingHighlights(
    QList<SearchFeatures::HighlightInfo>& highlights) {
    for (int i = highlights.size() - 1; i >= 0; --i) {
        for (int j = i - 1; j >= 0; --j) {
            if (highlights[i].rect.intersects(highlights[j].rect)) {
                ...
                highlights[j].priority =
                    qMax(highlights[i].priority, highlights[j].priority);
                highlights[j].isCurrentResult = highlights[i].isCurrentResult ||
                                                highlights[j].isCurrentResult;

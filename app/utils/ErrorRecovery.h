...
struct RecoveryContext {
    ...
    RecoveryContext(const ErrorHandling::ErrorInfo& err, const QString& component, const QString& operation)
        : error(err), componentName(component), operationName(operation), firstAttempt(QDateTime::currentDateTime()) {}
};
...
public:
    ...
    auto retryWithPolicy(Func&& func, const RetryConfig& config,
                        const QString& context = QString()) -> ErrorHandling::Result<decltype(func())> {
        using ReturnType = decltype(func());
        using ResultType = ErrorHandling::Result<ReturnType>;

        if (config.policy == RetryPolicy::None || config.maxAttempts <= 0) {
            return ErrorHandling::safeExecute(std::forward<Func>(func),
                                            ErrorHandling::ErrorCategory::Unknown, context);
        }

        ErrorHandling::ErrorInfo lastError;

        for (int attempt = 1; attempt <= config.maxAttempts; ++attempt) {
            auto result = ErrorHandling::safeExecute(std::forward<Func>(func),
                                                   ErrorHandling::ErrorCategory::Unknown, context);

            if (ErrorHandling::isSuccess(result)) {
                if (attempt > 1) {
                    Logger::instance().info("Retry succeeded on attempt {} for: {}",
                                          attempt, context.toStdString());
                }
                return result;
            }
            ...
            if (attempt < config.maxAttempts) {
                ...
                if (delay.count() > 0) {
                    Logger::instance().info("Retrying in {}ms (attempt {}/{}) for: {}",
                                          delay.count(), attempt, config.maxAttempts, context.toStdString());
                    QThread::msleep(static_cast<unsigned long>(delay.count()));
                }
            }
        }

        Logger::instance().warning("All retry attempts failed for: {}", context.toStdString());
    ...
    // Configuration
    void setDefaultRetryConfig(const RetryConfig& config) { m_defaultRetryConfig = config; }
    const RetryConfig& getDefaultRetryConfig() const { return m_defaultRetryConfig; }
...
signals:
    void recoveryAttempted(const QString& component, const QString& operation, RecoveryResult result);
    void circuitBreakerStateChanged(const QString& name, CircuitState oldState, CircuitState newState);

private:
    RecoveryManager() = default;
    
    std::chrono::milliseconds calculateDelay(const RetryConfig& config, int attempt) const;
    void updateStats(const QString& componentName, RecoveryResult result);
    
    std::map<ErrorHandling::ErrorCategory, std::shared_ptr<IRecoveryAction>> m_recoveryActions;
...
public:
    ...
    QString getDescription() const override { return "File system error recovery"; }
};

class DocumentRecoveryAction : public IRecoveryAction {
public:
    RecoveryResult execute(const ErrorHandling::ErrorInfo& error) override;
    QString getDescription() const override { return "Document error recovery"; }
};

class RenderingRecoveryAction : public IRecoveryAction {
public:
    RecoveryResult execute(const ErrorHandling::ErrorInfo& error) override;
    QString getDescription() const override { return "Rendering error recovery"; }
};
...
namespace Utils {
    ...
    void safeCleanup(Resource& resource, Cleanup&& cleanup, const QString& context = QString());
    ...
    public:
        ...
        ~StateGuard() { if (!m_committed) m_state = m_originalState; }
        void commit() { m_committed = true; }
        void rollback() { m_state = m_originalState; m_committed = true; }
...
#define RETRY_ON_ERROR(func, config) \
    ErrorRecovery::RecoveryManager::instance().retryWithPolicy([&]() { return func; }, config, #func)

#define WITH_CIRCUIT_BREAKER(name, func) \
    [&]() { \
        auto& breaker = ErrorRecovery::RecoveryManager::instance().getCircuitBreaker(name); \
        if (!breaker.canExecute()) { \
            throw ErrorHandling::ApplicationException(ErrorHandling::ErrorCategory::Unknown, \

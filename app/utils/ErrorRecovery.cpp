...
CircuitBreaker::CircuitBreaker(int failureThreshold, std::chrono::milliseconds timeout)
...
void CircuitBreaker::recordFailure() {
    ...
    if (m_state == CircuitState::Closed && m_failureCount >= m_failureThreshold) {
        transitionToOpen();
...
void CircuitBreaker::transitionToOpen() {
    ...
    Logger::instance().warning("Circuit breaker transitioned to OPEN state (failures: {})", m_failureCount.load());
}
...
void RecoveryManager::registerRecoveryAction(ErrorHandling::ErrorCategory category, 
                                            std::shared_ptr<IRecoveryAction> action) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_recoveryActions[category] = action;
    Logger::instance().info("Registered recovery action for category: {}", 
                           ErrorHandling::categoryToString(category).toStdString());
}

RecoveryResult RecoveryManager::executeRecovery(const ErrorHandling::ErrorInfo& error,
    ...
    if (it == m_recoveryActions.end()) {
        Logger::instance().warning("No recovery action registered for category: {}", 
    ...
    try {
        Logger::instance().info("Executing recovery for {}.{}: {}", 
                               componentName.toStdString(), operationName.toStdString(), 
        ...
        Logger::instance().error("Recovery action failed with exception: {}", e.what());
        updateStats(componentName, RecoveryResult::Failed);
        return RecoveryResult::Failed;
    }
}
...
CircuitBreaker& RecoveryManager::getCircuitBreaker(const QString& name) {
    ...
    if (it == m_circuitBreakers.end()) {
        ...
        Logger::instance().info("Created circuit breaker: {}", name.toStdString());
    }
...
void RecoveryManager::resetCircuitBreaker(const QString& name) {
    ...
    if (it != m_circuitBreakers.end()) {
        it->second->reset();
        Logger::instance().info("Reset circuit breaker: {}", name.toStdString());
    }
}

std::chrono::milliseconds RecoveryManager::calculateDelay(const RetryConfig& config, int attempt) const {
    switch (config.policy) {
        ...
        case RetryPolicy::ExponentialBackoff: {
            auto delay = config.initialDelay * static_cast<long long>(std::pow(config.backoffMultiplier, attempt - 1));
...
void RecoveryManager::updateStats(const QString& componentName, RecoveryResult result) {
...
RecoveryManager::RecoveryStats RecoveryManager::getStats(const QString& componentName) const {
    std::lock_guard<std::mutex> lock(m_mutex);
...
RecoveryResult FileSystemRecoveryAction::execute(const ErrorHandling::ErrorInfo& error) {
    Logger::instance().info("Attempting file system recovery for: {}", error.message.toStdString());
    ...
    if (!fileInfo.dir().exists()) {
        if (fileInfo.dir().mkpath(".")) {
            Logger::instance().info("Created missing directory: {}", fileInfo.dir().path().toStdString());
            return RecoveryResult::Retry;
        }
    }
    
    // Check if file is locked and suggest retry
    if (error.message.contains("locked") || error.message.contains("access denied")) {
...
RecoveryResult DocumentRecoveryAction::execute(const ErrorHandling::ErrorInfo& error) {
    Logger::instance().info("Attempting document recovery for: {}", error.message.toStdString());
    ...
    if (error.message.contains("memory") || error.message.contains("allocation")) {
...
RecoveryResult RenderingRecoveryAction::execute(const ErrorHandling::ErrorInfo& error) {
    Logger::instance().info("Attempting rendering recovery for: {}", error.message.toStdString());
    ...
    if (fullErrorText.contains("DPI", Qt::CaseInsensitive) ||
        ...
        Logger::instance().info("DPI/resolution issue detected, suggesting fallback");
        return RecoveryResult::Fallback;
    }
    ...
    Logger::instance().warning("No specific recovery strategy for rendering error: {}", error.message.toStdString());
    return RecoveryResult::Failed;
}

RecoveryResult SearchRecoveryAction::execute(const ErrorHandling::ErrorInfo& error) {
    Logger::instance().info("Attempting search recovery for: {}", error.message.toStdString());
...
RecoveryResult CacheRecoveryAction::execute(const ErrorHandling::ErrorInfo& error) {
    Logger::instance().info("Attempting cache recovery for: {}", error.message.toStdString());
...
namespace Utils {
    ...
    RetryConfig createStandardRetry() {
        return RetryConfig(RetryPolicy::ExponentialBackoff, 3, std::chrono::milliseconds(100));
    }
    
    RetryConfig createPatientRetry() {
        return RetryConfig(RetryPolicy::ExponentialBackoff, 5, std::chrono::milliseconds(500));
    }
    
    RetryConfig createNetworkRetry() {
        return RetryConfig(RetryPolicy::ExponentialBackoff, 4, std::chrono::milliseconds(1000));
    }
}

...
QString categoryToString(ErrorCategory category) {
    switch (category) {
        ...
        case ErrorCategory::Document:      return "Document";
        case ErrorCategory::Rendering:     return "Rendering";
        case ErrorCategory::Search:        return "Search";
...
ErrorInfo createFileSystemError(const QString& operation, const QString& path, const QString& details) {
    return ErrorInfo(
        ErrorCategory::FileSystem,
        ErrorSeverity::Error,
        QString("File system operation failed: %1").arg(operation),
        details.isEmpty() ? QString("Path: %1").arg(path) : QString("Path: %1, %2").arg(path, details),
        operation
    );
}

ErrorInfo createDocumentError(const QString& operation, const QString& details) {
    return ErrorInfo(
...
ErrorInfo createRenderingError(const QString& operation, const QString& details) {
    return ErrorInfo(
...
ErrorInfo createCacheError(const QString& operation, const QString& details) {
    return ErrorInfo(
        ...
        QString("Cache operation failed: %1").arg(operation),
        details,
        operation
    );
}

ErrorInfo createThreadingError(const QString& operation, const QString& details) {
    return ErrorInfo(
        ErrorCategory::Threading,
        ErrorSeverity::Critical,  // Threading errors are usually serious
        QString("Threading operation failed: %1").arg(operation),
        details,
        operation
    );
}

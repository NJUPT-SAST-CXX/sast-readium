...
public:
    ...
    QObject* createModel(const QString& type,
...
void TestModelFactory::testFactoryPriority() {
    ...
    QObject* model =
        priorityList.first().factory->createModel("common", QVariantMap());
...
void TestModelFactory::testModelPooling() {
    ...
    public:
        ...
        ~ModelPool() { qDeleteAll(m_available); }
...
void TestModelFactory::testBuilderPattern() {
    ...
    QObject* model = builder.withId("builder-123")
...
void TestModelFactory::testFactoryWithStateManager() {
    ...
    // Read configuration
    QString defaultType =
        StateManager::instance().get("factory.defaultType").toString();
    bool cacheEnabled =
        StateManager::instance().get("factory.cacheEnabled").toBool();
...
void TestModelFactory::testFactoryWithEventBus() {
    ...
    auto createWithEvent = [this](const QString& type,
...
void TestModelFactory::testLazyCreation() {
    ...
    class LazyModel {
    public:
        LazyModel(MockModelFactory* factory, const QString& type,
                  const QVariantMap& params)
...
void TestModelFactory::testCachePerformance() {
    ...
    qint64 elapsed = timer.elapsed();
    qDebug() << "Cache performance:" << numAccesses << "accesses in" << elapsed
             << "ms";

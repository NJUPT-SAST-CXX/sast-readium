...
private slots:
    ...
    void testActionMapUniqueness() {
        ...
        actionValues.insert(
            static_cast<int>(ActionMap::setContinuousScrollMode));
        ...
        QVERIFY(actionValues.size() >=
                40);  // Allow some flexibility for enum changes
    }
    ...
    void testActionMapComparison() {
        ...
        QVERIFY(static_cast<int>(ActionMap::openFile) <
                static_cast<int>(ActionMap::reloadFile));
    }
    ...
    void testActionMapBoundaryValues() {
        ...
        QVERIFY(reloadFileValue >= 40);  // Should be a reasonably high value
                                         // (reloadFile is at position 43)

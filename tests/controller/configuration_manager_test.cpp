...
private slots:
    ...
    void testSetAndGetValue() {
        ...
        QSignalSpy configChangedSpy(&config,
            static_cast<void(ConfigurationManager::*)(const QString&, const QVariant&)>(&ConfigurationManager::configurationChanged));
    ...
    void testDefaultValues() {
        ...
        QCOMPARE(config.getValue("nonexistent.key", "default").toString(), QString("default"));
    ...
    void testGroupBasedOperations() {
        ...
        QSignalSpy groupConfigChangedSpy(&config, 
            QOverload<ConfigurationManager::ConfigGroup, const QString&, const QVariant&>::of(
                &ConfigurationManager::configurationChanged));
        
        // Test group-based operations
        config.setValue(ConfigurationManager::UI, "theme", "dark");
        QCOMPARE(config.getValue(ConfigurationManager::UI, "theme").toString(), QString("dark"));
        ...
        QCOMPARE(arguments.at(0).toInt(), static_cast<int>(ConfigurationManager::UI));
    ...
    void testTypeSafeAccessors() {
        ...
        QCOMPARE(config.getString("nonexistent.string", "world"), QString("world"));
        ...
        QCOMPARE(config.getStringList("nonexistent.stringlist", defaultList), defaultList);
    }
    ...
    void testSaveAndLoadConfiguration() {
        ...
        QSignalSpy loadedSpy(&config, &ConfigurationManager::configurationLoaded);
        ...
        QCOMPARE(config.getValue("test.save").toString(), QString("saved_value"));
        QCOMPARE(config.getValue(ConfigurationManager::General, "app.version").toString(), QString("1.0.0"));
    }
    ...
    void testResetToDefaults() {
        ...
        QCOMPARE(config.getValue("test.reset").toString(), QString("will_be_reset"));
        ...
        QCOMPARE(config.getValue("test.reset", "default").toString(), QString("default"));
    }
    ...
    void testResetGroup() {
        ...
        QCOMPARE(config.getValue(ConfigurationManager::UI, "theme", "light").toString(), QString("light"));
        QCOMPARE(config.getValue(ConfigurationManager::UI, "font_size", 10).toInt(), 10);
        QCOMPARE(config.getValue(ConfigurationManager::General, "language").toString(), QString("en"));
    }
    ...
    void testRuntimeValues() {
        ...
        QCOMPARE(config.getRuntimeValue("runtime.test").toString(), QString("runtime_value"));
        QCOMPARE(config.getRuntimeValue("nonexistent.runtime", "default").toString(), QString("default"));
        
        // Runtime values should not affect persistent configuration
        QCOMPARE(config.getValue("runtime.test", "not_found").toString(), QString("not_found"));
        
        // Clear runtime values
        config.clearRuntimeValues();
        QCOMPARE(config.getRuntimeValue("runtime.test", "cleared").toString(), QString("cleared"));
    }
    ...
    void testImportExportConfiguration() {
        ...
        // Reset configuration
        config.resetToDefaults();
        QCOMPARE(config.getValue("export.test1", "not_found").toString(), QString("not_found"));
        ...
        QCOMPARE(config.getValue(ConfigurationManager::UI, "export.theme").toString(), QString("dark"));
    }
    ...
    void testConfigurationValidation() {
        ...
        QSignalSpy validationFailedSpy(&config, &ConfigurationManager::validationFailed);
    ...
    void testConvenienceMacros() {
        ...
        QCOMPARE(CONFIG_GET("macro.test", "not_found").toString(), QString("macro_value"));

...
public:
    ...
    Event* clone() const override { return new TestEvent(m_message); }
...
class TestSubscriber : public EventSubscriber {
    Q_OBJECT
public:
    explicit TestSubscriber(QObject* parent = nullptr)
        : EventSubscriber(parent) {}
...
private slots:
    ...
    void testSubscriptionManagement() {
        ...
        QSignalSpy subscriberRemovedSpy(&eventBus,
                                        &EventBus::subscriberRemoved);
    ...
    void testMultipleSubscribers() {
        ...
        eventBus.subscribe("multi.event", this,
                           [&](Event*) { subscriber1Count++; });

        TestSubscriber* subscriber2 = new TestSubscriber(this);
        eventBus.subscribe("multi.event", subscriber2,
                           [&](Event*) { subscriber2Count++; });
    ...
    void testEventFiltering() {
        ...
        // Subscribe to events
        eventBus.subscribe("filter.event", this,
    ...
    void testAsyncEventPublishing() {
        ...
        // Subscribe to events
        eventBus.subscribe("async.event", this,
                           [&](Event*) { eventReceived = true; });
    ...
    void testTypedEvent() {
        ...
        TypedEvent<TestData>* typedEvent =
            new TypedEvent<TestData>("typed.event", testData);
        ...
        TypedEvent<TestData>* clonedTypedEvent =
            dynamic_cast<TypedEvent<TestData>*>(clonedEvent);
    ...
    void testEventSubscriber() {
        ...
        QCOMPARE(m_testSubscriber->receivedEvents().first()->data().toString(),
                 QString("subscriber_test"));
    }

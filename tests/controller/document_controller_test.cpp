...
public:
    explicit MockDocumentModel(QObject* parent = nullptr)
        : DocumentModel(nullptr), m_isEmpty(true), m_documentCount(0), m_currentIndex(-1) {
        setParent(parent);
    }
...
class MockRecentFilesManager : public RecentFilesManager {
    Q_OBJECT
public:
    explicit MockRecentFilesManager(QObject* parent = nullptr) : RecentFilesManager(parent) {}
...
private slots:
    ...
    void testConstructorWithValidModel() {
        ...
        QVERIFY(m_documentController->getDocumentModel() == m_mockDocumentModel);
        QVERIFY(m_documentController->getRecentFilesManager() == m_mockRecentFilesManager);
    }
    ...
    void testOpenValidDocument() {
        QSignalSpy operationSpy(m_documentController,
        ...
        qDebug() << "MockDocumentModel document count:" << m_mockDocumentModel->getDocumentCount();
        qDebug() << "MockDocumentModel current file:" << m_mockDocumentModel->getCurrentFilePath();
        ...
        QCOMPARE(recentFilesSpy.first().first().toString(), QString("test.pdf"));
    }
    ...
    void testViewModeChangeSignal() {
        ...
        m_documentController->execute(ActionMap::setContinuousScrollMode, nullptr);
...
#include "document_controller_test.moc"

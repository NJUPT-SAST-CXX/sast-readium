...
void TestStateManager::testStateChangeSignals() {
    QSignalSpy changeSpy(m_stateManager,
        static_cast<void(StateManager::*)(const QString&, const QVariant&, const QVariant&)>(&StateManager::stateChanged));
...
void TestStateManager::testWildcardSubscriptions() {
    ...
    for (const auto& paths : changedPaths) {
        if (paths.contains("app.setting1")) hasAppSetting1 = true;
        if (paths.contains("app.setting2")) hasAppSetting2 = true;
    }
...
void TestStateManager::testStateManagerWithEventBus() {
    ...
    // Connect StateManager to EventBus - use function pointer syntax to resolve ambiguity
    QObject::connect(m_stateManager,
        static_cast<void(StateManager::*)(const StateChange&)>(&StateManager::stateChanged),

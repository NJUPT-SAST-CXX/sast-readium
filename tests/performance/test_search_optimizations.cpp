...
Poppler::Document* TestSearchOptimizations::createTestDocument() {
    ...
    QStringList testTexts = {
        "This is the first page with some sample text for searching. The quick "
        "brown fox jumps over the lazy dog.",
        "Second page contains different content. Lorem ipsum dolor sit amet, "
        "consectetur adipiscing elit.",
        "Third page has more text to search through. Sed do eiusmod tempor "
        "incididunt ut labore et dolore magna aliqua.",
        "Fourth page with additional content. Ut enim ad minim veniam, quis "
        "nostrud exercitation ullamco laboris.",
        "Fifth page continues the pattern. Duis aute irure dolor in "
        "reprehenderit in voluptate velit esse cillum.",
        "Sixth page has even more text. Excepteur sint occaecat cupidatat non "
        "proident, sunt in culpa qui officia.",
        "Seventh page with unique content. Deserunt mollit anim id est "
        "laborum. Sed ut perspiciatis unde omnis.",
        "Eighth page contains special keywords: optimization, performance, "
        "cache, search, engine, fast, efficient.",
        "Ninth page has repeated words: test test test search search search "
        "performance performance cache cache.",
        "Tenth page concludes with final text. At vero eos et accusamus et "
...
void TestSearchOptimizations::testSearchResultCachePerformance() {
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        for (int j = 0; j < 10; ++j) {
            SearchResult result(j, key.query, "context", QRectF(0, 0, 100, 20),
                                0, key.query.length());
    ...
    QVERIFY(storeTime <
            1000);  // Should store 1000 entries in less than 1 second
    QVERIFY(retrieveTime <
            500);  // Should retrieve 1000 entries in less than 0.5 seconds
...
void TestSearchOptimizations::testPageTextCachePerformance() {
    ...
    timer.start();
    for (int i = 0; i < 100; ++i) {
        QString text = QString("This is test text for page %1. ")
                           .repeated(100);  // ~3KB per page
    ...
    qDebug() << "Page text cache memory usage:" << cache.getMemoryUsage()
             << "bytes";

    QVERIFY(storeTime <
            500);  // Should store 100 pages in less than 0.5 seconds
    QVERIFY(retrieveTime <
            100);          // Should retrieve 100 pages in less than 0.1 seconds
...
void TestSearchOptimizations::testOptimizedSearchEnginePerformance() {
    ...
    QElapsedTimer timer;
    QStringList testQueries = {"test",         "search", "performance",
                               "optimization", "quick",  "lorem"};
    ...
    qDebug() << "Performance improvement:"
             << (double)coldSearchTime / warmSearchTime << "x";
...
void TestSearchOptimizations::testIncrementalSearchPerformance() {
    ...
    QVERIFY(incrementalTime <=
            regularTime);  // Incremental should be at least as fast
}

void TestSearchOptimizations::testOptimizedVsBasicSearchPerformance() {
    qDebug() << "Testing optimized vs basic search performance";

    QStringList testQueries = {"test", "search", "performance", "quick",
                               "lorem"};
    ...
    QVERIFY(improvement >=
            1.0);  // Optimized should be at least as fast as basic
}
...
void TestSearchOptimizations::testSearchResponseTime() {
    ...
    for (const QString& query : queries) {
        ...
        qDebug() << "Query:" << query << "- Response time:" << responseTime
                 << "ms";
    }
...
void TestSearchOptimizations::performanceComparison(const QString& query,
                                                    int iterations) {

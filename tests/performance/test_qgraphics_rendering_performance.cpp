...
private:
    ...
    PerformanceMetrics measureQGraphicsRendering(QGraphicsPDFViewer* viewer,
                                                 int pageNum);
...
TestRenderingPerformance::PerformanceMetrics
TestRenderingPerformance::measureQGraphicsRendering(QGraphicsPDFViewer* viewer,
                                                    int pageNum) {
    PerformanceMetrics metrics = {0, 0, 0, 0, 0.0, 0};
...
void TestRenderingPerformance::testQGraphicsVsTraditionalRendering() {
    ...
    // Measure QGraphics rendering
    auto qgraphicsMetrics =
        measureQGraphicsRendering(m_qgraphicsViewer, testPage);
    ...
    double speedup =
        (double)traditionalMetrics.renderTime / qgraphicsMetrics.renderTime;
...
void TestRenderingPerformance::testMemoryUsageComparison() {
    ...
    QVERIFY(qgraphicsMemory <
            traditionalMemory * 2);  // Not more than 2x memory
}
...
void TestRenderingPerformance::testPrerenderingMemoryUsage() {
    ...
    qDebug() << "Prerendering memory usage:" << memoryUsed / (1024 * 1024)
             << "MB for 10 pages";
...
void TestRenderingPerformance::testAdaptivePrerendering() {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        m_prerenderer->adaptivePrerender(i,
                                         1);  // Current page, direction forward
        waitMs(50);

        qint64 adaptiveTime = timer.elapsed();
        qDebug() << "Adaptive prerender for page" << i << ":" << adaptiveTime
                 << "ms";
...
void TestRenderingPerformance::benchmarkCacheLookup() {
    ...
    double avgLookup =
        (double)lookupTime / lookups * 1000;  // Convert to microseconds

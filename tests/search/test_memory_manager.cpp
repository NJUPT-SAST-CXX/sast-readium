...
void MemoryManagerTest::testOptimizationPerformance() {
    ...
    // Optimization should complete within reasonable time (< 1 second)
    QVERIFY(elapsed < 1000);
}

void MemoryManagerTest::testMemoryLeakPrevention() {
    // Get initial memory stats
    MemoryManager::MemoryStats initialStats = m_manager->getMemoryStats();
    ...
    MemoryManager::MemoryStats finalStats = m_manager->getMemoryStats();

    // Memory usage should not grow unbounded
    // Allow some growth but not excessive (< 10MB increase)
    qint64 memoryGrowth =
        finalStats.totalMemoryUsage - initialStats.totalMemoryUsage;
    QVERIFY(memoryGrowth < 10 * 1024 * 1024);
}

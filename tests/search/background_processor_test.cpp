...
void BackgroundProcessorTest::testExecuteAsync()
{
    ...
    m_processor->executeAsync([&taskExecuted]() {
        taskExecuted = true;
    });
...
{
    auto future = m_processor->execute<int>([]() -> int {
        return 42;
    });
...
void BackgroundProcessorTest::waitForTaskCompletion(int expectedTasks, int timeoutMs)
{
    QElapsedTimer timer;
    timer.start();

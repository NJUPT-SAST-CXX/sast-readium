...
private:
    ...
    void verifySuggestionQuality(const QStringList& suggestions, const QString& prefix);
...
void MultiSearchEngineTest::testConstructor()
{
    ...
    QStringList suggestions = m_suggestionEngine->generateSuggestions("test", 5);
...
void MultiSearchEngineTest::testTrainModel()
{
    ...
    QStringList suggestions = m_suggestionEngine->generateSuggestions("search", 5);
    QVERIFY(!suggestions.isEmpty());
    ...
    for (const QString& suggestion : suggestions) {
        if (suggestion == "search engine") foundSearchEngine = true;
        if (suggestion == "search algorithm") foundSearchAlgorithm = true;
    }
...
void MultiSearchEngineTest::testGenerateSuggestionsWithPrefix()
{
    ...
    QStringList searchSuggestions = m_suggestionEngine->generateSuggestions("search", 3);
    QStringList textSuggestions = m_suggestionEngine->generateSuggestions("text", 3);
    QStringList fuzzySuggestions = m_suggestionEngine->generateSuggestions("fuzzy", 3);
...
void MultiSearchEngineTest::testGenerateSuggestionsWithLimit()
{
    ...
    QStringList suggestions1 = m_suggestionEngine->generateSuggestions("search", 1);
    QStringList suggestions3 = m_suggestionEngine->generateSuggestions("search", 3);
    QStringList suggestions10 = m_suggestionEngine->generateSuggestions("search", 10);
...
void MultiSearchEngineTest::testGetQueryFrequency()
{
    ...
    int nonExistentFrequency = m_suggestionEngine->getQueryFrequency("non-existent query");
...
void MultiSearchEngineTest::testSuggestionRanking()
{
    ...
    QStringList suggestions = m_suggestionEngine->generateSuggestions("search", 3);
...
QStringList MultiSearchEngineTest::generateRandomQueries(int count, int averageLength)
{
    QStringList queries;
    const QStringList words = {"search", "find", "query", "text", "document", "file", 
                              "content", "algorithm", "engine", "system", "data", "information"};
    ...
    for (int i = 0; i < count; ++i) {
        ...
        for (int j = 0; j < wordCount; ++j) {
            queryWords.append(words[QRandomGenerator::global()->bounded(words.size())]);
        }
...
QList<int> MultiSearchEngineTest::generateRandomFrequencies(int count, int maxFrequency)
{
    QList<int> frequencies;
    
    for (int i = 0; i < count; ++i) {
        frequencies.append(QRandomGenerator::global()->bounded(1, maxFrequency + 1));
    }
    
    return frequencies;
}

void MultiSearchEngineTest::verifySuggestionQuality(const QStringList& suggestions, const QString& prefix)
{
    for (const QString& suggestion : suggestions) {

...
private:
    ...
    SearchErrorRecovery::ErrorContext createErrorContext(SearchErrorRecovery::ErrorType type, 
...
void SearchErrorRecoveryTest::setupTestContext()
{
    m_testContext = SearchErrorRecovery::ErrorContext(
        SearchErrorRecovery::SearchError,
        "test_operation",
        "test_component",
        "Test error details"
    );
...
void SearchErrorRecoveryTest::testConstructor()
{
    ...
    auto config = m_recovery->getRecoveryConfig(SearchErrorRecovery::SearchError);
...
{
    m_recovery->setRecoveryConfig(SearchErrorRecovery::SearchError, m_testConfig);
    
    auto retrievedConfig = m_recovery->getRecoveryConfig(SearchErrorRecovery::SearchError);
    ...
    QCOMPARE(retrievedConfig.exponentialBackoff, m_testConfig.exponentialBackoff);
}

void SearchErrorRecoveryTest::testGetRecoveryConfig()
{
    // Test getting config for different error types
    auto searchConfig = m_recovery->getRecoveryConfig(SearchErrorRecovery::SearchError);
    auto documentConfig = m_recovery->getRecoveryConfig(SearchErrorRecovery::DocumentError);
    auto cacheConfig = m_recovery->getRecoveryConfig(SearchErrorRecovery::CacheError);
...
{
    auto fallbackFunction = [](const SearchErrorRecovery::ErrorContext& context) -> QVariant {
        Q_UNUSED(context)
        return QVariant("Fallback result");
    };

    m_recovery->registerFallback(SearchErrorRecovery::SearchError, "test_operation", fallbackFunction);
...
{
    m_recovery->unregisterFallback(SearchErrorRecovery::SearchError, "test_operation");
...
void SearchErrorRecoveryTest::testExecuteFallback()
{
    ...
    QVariant result = m_recovery->executeFallback(SearchErrorRecovery::SearchError, "test_operation", m_testContext);
...
{
    QSignalSpy healthSpy(m_recovery, &SearchErrorRecovery::componentHealthChanged);
...
{
    QSignalSpy attemptedSpy(m_recovery, &SearchErrorRecovery::recoveryAttempted);
    QSignalSpy succeededSpy(m_recovery, &SearchErrorRecovery::recoverySucceeded);
...
{
    QSignalSpy openedSpy(m_recovery, &SearchErrorRecovery::circuitBreakerOpened);
    QSignalSpy closedSpy(m_recovery, &SearchErrorRecovery::circuitBreakerClosed);
...
void SearchErrorRecoveryTest::testSearchException()
{
    ...
    QCOMPARE(QString::fromStdString(exception.what()), QString("Test search exception"));
...
void SearchErrorRecoveryTest::testDocumentException()
{
    ...
    QCOMPARE(QString::fromStdString(exception.what()), QString("Test document exception"));
...
void SearchErrorRecoveryTest::testCacheException()
{
    ...
    QCOMPARE(QString::fromStdString(exception.what()), QString("Test cache exception"));
...
void SearchErrorRecoveryTest::testTimeoutException()
{
    ...
    QCOMPARE(QString::fromStdString(exception.what()), QString("Test timeout exception"));
...
{
    return SearchErrorRecovery::ErrorContext(type, operation, "test_component", "Test details");
}
...
bool SearchErrorRecoveryTest::successfulOperation()
{
    return true;
}

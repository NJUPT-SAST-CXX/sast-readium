 ...
 * Tests atomic operations, shared data access, thread-safe containers, and synchronization
 */
...
private:
    ...
    // Helper methods
    void runConcurrentTest(std::function<void(int)> threadFunction, int threadCount = THREAD_COUNT);
...
void SearchThreadSafetyTest::initTestCase()
{
    ...
    qDebug() << "Using" << THREAD_COUNT << "threads with" << ITERATIONS_PER_THREAD << "iterations each";
}
...
void SearchThreadSafetyTest::testThreadSafeQueueProducerConsumer()
{
    ...
    auto consumer = [&queue, &consumedCount](int threadId) {
        Q_UNUSED(threadId)
        int value;
        while (consumedCount.loadAcquire() < THREAD_COUNT * ITERATIONS_PER_THREAD) {
    ...
    QCOMPARE(producedCount.loadAcquire(), THREAD_COUNT / 2 * ITERATIONS_PER_THREAD);
    QCOMPARE(consumedCount.loadAcquire(), THREAD_COUNT / 2 * ITERATIONS_PER_THREAD);
}
...
void SearchThreadSafetyTest::runConcurrentTest(std::function<void(int)> threadFunction, int threadCount)
{
    QList<QFuture<void>> futures;

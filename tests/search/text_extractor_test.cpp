...
private:
    ...
    void verifyExtractedText(const QString& text, const QString& expectedContent);
    void measureExtractionTime(int pageNumber);
};

void TextExtractorTest::initTestCase()
{
    createTestPdf();
}
...
void TextExtractorTest::testCacheEfficiency()
{
    ...
    QCOMPARE(text1, text2);
    // Cache should be faster (though this might not always be true for small documents)
    // QVERIFY(time2 <= time1);
}
...
void TextExtractorTest::createTestPdf()
{
    ...
    painter.drawText(100, 200, "It contains multiple lines of text for testing.");
    painter.drawText(100, 300, "The extractor should be able to extract this text efficiently.");
...
void TextExtractorTest::verifyExtractedText(const QString& text, const QString& expectedContent)
{
    QVERIFY(!text.isEmpty());
...
void TextExtractorTest::measureExtractionTime(int pageNumber)
{
    ...
    qDebug() << "Extraction time for page" << pageNumber << ":" << extractionTime << "ms";
    QVERIFY(!text.isEmpty());

...
private:
    ...
    void verifySearchOptions(const SearchOptions& options, bool caseSensitive, bool wholeWords);
    void verifySearchResult(const SearchResult& result, int page, const QString& text);
};
...
void SearchConfigurationTest::testSearchResultConstruction()
{
    ...
    SearchResult result2(1, "test", "This is a test", QRectF(10, 10, 50, 20), 10, 4);
...
void SearchConfigurationTest::testSearchResultValidation()
{
    ...
    result.pageNumber = 1;
    QVERIFY(result.isValid());
    QVERIFY(!result.isValidForHighlight()); // Still invalid for highlight (empty rect)
...
void SearchConfigurationTest::testSearchResultTransformation()
{
    ...
    result.transformToWidgetCoordinates(scaleFactor, rotation, pageSize, widgetSize);
...
void SearchConfigurationTest::verifySearchOptions(const SearchOptions& options, bool caseSensitive, bool wholeWords)
{
    QCOMPARE(options.caseSensitive, caseSensitive);
    QCOMPARE(options.wholeWords, wholeWords);
}

void SearchConfigurationTest::verifySearchResult(const SearchResult& result, int page, const QString& text)
{

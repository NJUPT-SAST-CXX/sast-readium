...
void TestSearchEngineCore::testCaseSensitiveSearch() {
    ...
    for (const SearchResult& result : results) {
        QVERIFY(result.text.contains("TEST"));
    }
}
...
void TestSearchEngineCore::testCaseInsensitiveSearch() {
    ...
    for (const SearchResult& result : results) {
        QVERIFY(result.text.contains("test", Qt::CaseInsensitive));
    }
}
...
void TestSearchEngineCore::testFuzzySearch() {
    ...
    for (const SearchResult& result : results) {
        QVERIFY(result.text.contains("document", Qt::CaseInsensitive));
    }
}
...
void TestSearchEngineCore::testSearchResultAccuracy() {
    ...
    QVERIFY(result.text.contains("quick", Qt::CaseInsensitive));
    QVERIFY(result.startIndex >= 0);
...
#include "test_search_engine_core.moc"

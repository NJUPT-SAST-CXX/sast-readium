 ...
 * Tests with empty documents, malformed PDFs, large documents, and error scenarios
 */
...
Poppler::Document* TestSearchEdgeCases::createNormalTestDocument()
{
    ...
    QPainter painter(&pdfWriter);
    painter.drawText(100, 100, "Normal test document with content for edge case testing.");
    painter.end();
...
void TestSearchEdgeCases::testConcurrentSearchCancellation()
{
    ...
    for (int i = 0; i < 5; ++i) {
        m_searchEngine->startSearch(m_largeDocument, QString("test%1").arg(i), options);
...
void TestSearchEdgeCases::testLargeDocumentHandling()
{
    ...
    qDebug() << "Large document search: found" << results.size()
...
void TestSearchEdgeCases::testTooManySearchResults()
{
    ...
    qDebug() << "Max results test: found" << results.size() << "results (limit:" << options.maxResults << ")";
}
...
void TestSearchEdgeCases::testVeryFrequentSearches()
{
    ...
    for (int i = 0; i < 100; ++i) {
        QString query = QString("query%1").arg(i % 10); // Cycle through 10 queries
        m_searchEngine->startSearch(m_normalDocument, query, options);
...
{
    // This test simulates searching on a document with potentially corrupted pages
    SearchOptions options;
    ...
    for (int page = 0; page < m_normalDocument->numPages(); ++page) {
        ...
        m_searchModel->startPageRangeSearch(m_normalDocument, "test", page, page, options);
...
void TestSearchEdgeCases::testSearchWithMissingText()
{
    ...
    m_searchEngine->startSearch(m_normalDocument, "definitely_not_in_document_12345", options);

    QList<SearchResult> results = m_searchEngine->getResults();
    QVERIFY(results.isEmpty());

    // Test with various non-existent patterns
    QStringList nonExistentQueries = {
        "xyzabc123",
        "nonexistent_pattern_999",

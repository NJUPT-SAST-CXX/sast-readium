...
void TestSearchValidation::cleanupTestCase() { delete validator; }
...
void TestSearchValidation::testQueryLengthValidation() {
    ...
    result =
        strictValidator.validateQuery("this query is too long for the limit");
    QVERIFY(!result.isValid);
...
void TestSearchValidation::testRegexPatternValidation() {
    ...
    result = validator->validateQueryWithOptions("[invalid",
                                                 options);  // Invalid regex
    QVERIFY(!result.isValid);
...
void TestSearchValidation::testInvalidRegexDetection() {
    ...
    result =
        validator->validateQueryWithOptions("[unclosed bracket", regexOptions);
    QVERIFY(!result.isValid);
    QVERIFY(result.hasError(InvalidFormat));
}
...
void TestSearchValidation::testQuerySanitization() {
    QString query = QString("test%1with%2control%3chars")
                        .arg(QChar(0x00))
                        .arg(QChar(0x01))
                        .arg(QChar(0x1f));
...
void TestSearchValidation::testControlCharacterRemoval() {
    QString input = QString("normal%1text%2with%3control")
                        .arg(QChar(0x00))
                        .arg(QChar(0x01))
                        .arg(QChar(0x1f));
...
void TestSearchValidation::testMultipleQueryValidation() {
    QStringList queries = {"valid query", "", "another valid",
                           "javascript:alert()"};
...
void TestSearchValidation::testCustomValidationRules() {
    ...
    auto result =
        validator->applyCustomRules("test_rule", QVariant("normal text"));
    QVERIFY(result.isValid);

    result =
        validator->applyCustomRules("test_rule", QVariant("forbidden word"));
    QVERIFY(!result.isValid);

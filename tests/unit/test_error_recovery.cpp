...
void TestErrorRecovery::testCircuitBreaker()
{
    ...
    CircuitBreaker breaker(3, std::chrono::milliseconds(100)); // 3 failures, 100ms timeout
...
void TestErrorRecovery::testRecoveryActions()
{
    ...
    auto fsError = createFileSystemError("test operation", "/nonexistent/path", "File not found");
    auto fsResult = fsAction.execute(fsError);
    QVERIFY(fsResult == RecoveryResult::Failed || fsResult == RecoveryResult::Fallback);
...
void TestErrorRecovery::testRecoveryManager()
{
    ...
    auto result = manager.executeRecovery(error, "TestComponent", "testOperation");
    
    // Should attempt recovery (result depends on specific action implementation)
    QVERIFY(result == RecoveryResult::Failed || result == RecoveryResult::Fallback || 
            result == RecoveryResult::Retry || result == RecoveryResult::Success);
...
void TestErrorRecovery::testFileSystemRecovery()
{
    ...
    auto error = createFileSystemError("create file", testPath, "Directory does not exist");
    
    auto result = action.execute(error);
    // Should attempt to create directory and suggest retry
    QVERIFY(result == RecoveryResult::Retry || result == RecoveryResult::Failed);
}
...
void TestErrorRecovery::testRenderingRecovery()
{
    ...
    auto timeoutError = createRenderingError("render page", "Operation timeout");
...
void TestErrorRecovery::testRetryWithBackoff()
{
    ...
    int attempts = 0;
    auto config = RetryConfig(RetryPolicy::ExponentialBackoff, 3, std::chrono::milliseconds(10));
    
    auto result = manager.retryWithPolicy([&]() -> int {
        attempts++;
        if (attempts < 3) {

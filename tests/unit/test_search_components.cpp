...
void TestSearchComponents::testIncrementalManagerScheduling()
{
    ...
    QSignalSpy triggeredSpy(&manager, &IncrementalSearchManager::searchTriggered);
    QSignalSpy scheduledSpy(&manager, &IncrementalSearchManager::searchScheduled);
...
void TestSearchComponents::testIncrementalManagerRefinement()
{
    ...
    results.append(SearchResult(0, "testing", "Testing functionality", QRectF(), 0, 7));
    
    // Test refinement for extension
    QVERIFY(manager.canRefineSearch("testing", "test"));
    QList<SearchResult> refined = manager.refineResults(results, "testing", "test");
...
void TestSearchComponents::testIncrementalManagerCancellation()
{
    ...
    QSignalSpy cancelledSpy(&manager, &IncrementalSearchManager::searchCancelled);
...
void TestSearchComponents::testBackgroundProcessorBatch()
{
    ...
    QSignalSpy allFinishedSpy(&processor, &BackgroundProcessor::allTasksFinished);
...
void TestSearchComponents::testMetricsHistory()
{
    ...
    QList<SearchMetrics::Metric> rangeMetrics = metrics.metricsInRange(rangeStart, rangeEnd);

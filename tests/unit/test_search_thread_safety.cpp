...
void TestSearchThreadSafety::testMultiLockGuard() {
...

    {
        ...
        auto worker =
            new TestWorker([&mutex1]() { QVERIFY(!mutex1.tryLock()); });
        worker->start();
        worker->wait();
        delete worker;
    }
...
void TestSearchThreadSafety::testConcurrentPerformance() {
    ...
    for (int i = 0; i < numThreads; ++i) {
        auto worker =
            new TestWorker([&performanceCounter, operationsPerThread]() {
    ...
    qDebug() << "Performed" << totalOperations << "atomic operations in"
             << elapsed << "ms";
    qDebug() << "Rate:" << (totalOperations * 1000.0 / elapsed)
             << "operations/second";
}
...
void TestSearchThreadSafety::testScalability() {
    ...
    for (int threadCount = 1; threadCount <= maxThreads; threadCount *= 2) {
        ...
        for (int i = 0; i < threadCount; ++i) {
            auto worker =
                new TestWorker([&scalabilityCounter, operationsPerThread]() {
                    for (int j = 0; j < operationsPerThread; ++j) {
        ...
        qDebug() << "Threads:" << threadCount
                 << "Operations:" << totalOperations << "Time:" << elapsed
                 << "ms Rate:" << rate << "ops/sec";
    }
}

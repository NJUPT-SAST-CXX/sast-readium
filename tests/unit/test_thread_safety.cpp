...
TEST_F(ThreadSafetyTest, ThumbnailGeneratorConcurrentOperations) {
    ...
    generator.setDocument(std::shared_ptr<Poppler::Document>(
        document.release(), [](Poppler::Document* p) { delete p; }));
    generator.start();
    ...
    QObject::connect(
        &generator, &ThumbnailGenerator::thumbnailGenerated,
        [&completedThumbnails](int, const QPixmap&) { completedThumbnails++; });
    ...
    for (int i = 0; i < numThreads; ++i) {
        ...
        QObject::connect(
            thread.get(), &QThread::started, [&generator, requestsPerThread]() {
    ...
    EXPECT_LT(errorCount.load(), numThreads * requestsPerThread)
...
TEST_F(ThreadSafetyTest, AsyncDocumentLoaderConcurrentLoadCancel) {
    ...
    QObject::connect(
        &loader, &AsyncDocumentLoader::loadingCancelled,
        [&loadCancelledCount](const QString&) { loadCancelledCount++; });
    ...
    int totalOperations =
        loadCompletedCount + loadCancelledCount + loadFailedCount;
...
TEST_F(ThreadSafetyTest, PDFPrerendererThreadCoordination) {
    ...
    QObject::connect(
        &prerenderer, &PDFPrerenderer::pagePrerendered,
        [&prerenderedPages](int, double, int) { prerenderedPages++; });
...
TEST_F(ThreadSafetyTest, DocumentSwitchingUnderLoad) {
    ...
    ThumbnailGenerator generator;
    generator.setDocument(std::shared_ptr<Poppler::Document>(
        document1.release(), [](Poppler::Document* p) { delete p; }));
    generator.start();
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        if (newDocument) {
            generator.setDocument(std::shared_ptr<Poppler::Document>(
                newDocument.release(), [](Poppler::Document* p) { delete p; }));
        }
    }

...
class SearchTestRunner:
    ...
    def generate_report(
        self, results: Dict, output_file: str = "search_test_report.json"
    ):
        ...
        for suite_name, suite_data in results["suites"].items():
            for test_name, test_data in suite_data["tests"].items():
                if not test_data["success"]:
                    failed_tests.append(
                        (suite_name, test_name, test_data["output"][:100])
                    )
    ...
    def extract_performance_metrics(self, output: str):
        ...
        for line in lines:
            if "benchmark" in line.lower() or "performance" in line.lower():
                if any(
                    keyword in line for keyword in ["ms", "seconds", "time", "speed"]
                ):
...
def main():
    ...
    parser.add_argument(
        "--build-dir", default="build/Debug-MSYS2", help="Build directory path"
    )
    parser.add_argument(
        "--output", default="search_test_report.json", help="Output report file"
    )
    parser.add_argument(

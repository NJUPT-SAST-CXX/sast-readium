...
class MockWelcomeWidget : public WelcomeWidget {
    Q_OBJECT
public:
    MockWelcomeWidget(QWidget* parent = nullptr)
        : WelcomeWidget(parent), m_visible(false) {}
...
void WelcomeScreenManagerIntegrationTest::testInitialization() {
    ...
    // Test initial state
    QVERIFY(
        m_manager->isWelcomeScreenEnabled());  // Should be enabled by default
    QVERIFY(
        !m_manager
             ->isWelcomeScreenVisible());  // Should not be visible initially
}
...
void WelcomeScreenManagerIntegrationTest::testWelcomeScreenEnabled() {
    QSignalSpy enabledSpy(m_manager,
                          &WelcomeScreenManager::welcomeScreenEnabledChanged);
...
void WelcomeScreenManagerIntegrationTest::testWelcomeScreenVisibility() {
    QSignalSpy visibilitySpy(
        m_manager, &WelcomeScreenManager::welcomeScreenVisibilityChanged);
    QSignalSpy showRequestSpy(
        m_manager, &WelcomeScreenManager::showWelcomeScreenRequested);
    QSignalSpy hideRequestSpy(
        m_manager, &WelcomeScreenManager::hideWelcomeScreenRequested);
...
void WelcomeScreenManagerIntegrationTest::testDocumentModelIntegration() {
    QSignalSpy modelChangedSpy(m_manager,
...
#include "test_welcome_screen_manager_integration.moc"

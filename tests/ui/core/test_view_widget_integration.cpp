...
private slots:
    ...
    void testZoomLevelRetrieval();

    // Scroll position tests
    void testScrollPositionControl();
    void testScrollToTopBottom();

    // Document state tests
    void testDocumentStatePreservation();
    void testDocumentStateRestoration();
    void testGetSetDocumentState();

    // Error handling tests
    void testValidateDocumentIndex();
    void testValidateDocumentController();
    void testHandleDocumentError();

    // Unsaved changes tests
    void testHasUnsavedChanges();
    void testMarkDocumentModified();

    // Rotation tests
    void testRotationRetrieval();
...
void ViewWidgetIntegrationTest::testZoomControl() {
    if (!m_testPdfFile || !m_testPdfFile->exists()) {
        QSKIP("No test PDF file available");
    }
    ...
    if (m_viewWidget->hasDocuments()) {
        QSignalSpy zoomSpy(m_viewWidget, &ViewWidget::currentViewerZoomChanged);

        m_viewWidget->setZoom(1.5);
        waitForDocumentLoad();

        m_viewWidget->setZoom(0.75);
        waitForDocumentLoad();
...
void ViewWidgetIntegrationTest::testZoomLevelRetrieval() {
    if (!m_testPdfFile || !m_testPdfFile->exists()) {
        QSKIP("No test PDF file available");
    }
...
void ViewWidgetIntegrationTest::testScrollPositionControl() {
    if (!m_testPdfFile || !m_testPdfFile->exists()) {
        QSKIP("No test PDF file available");
    }
    ...
    if (m_viewWidget->hasDocuments()) {
        ...
        QVERIFY(pos.x() >= 0 || pos.x() < 0);  // Any value is valid

        m_viewWidget->setScrollPosition(QPoint(100, 200));
        waitForDocumentLoad();
        QVERIFY(true);
    }
}

void ViewWidgetIntegrationTest::testScrollToTopBottom() {
    if (!m_testPdfFile || !m_testPdfFile->exists()) {
        QSKIP("No test PDF file available");
    }
...
void ViewWidgetIntegrationTest::testDocumentStatePreservation() {
    if (!m_testPdfFile || !m_testPdfFile->exists()) {
        QSKIP("No test PDF file available");
    }
    ...
    if (m_viewWidget->hasDocuments()) {
        m_viewWidget->preserveCurrentDocumentState();
        QVERIFY(true);
    }
}

void ViewWidgetIntegrationTest::testDocumentStateRestoration() {
    if (!m_testPdfFile || !m_testPdfFile->exists()) {
        QSKIP("No test PDF file available");
    }
    ...
    if (m_viewWidget->hasDocuments()) {
        int index = m_viewWidget->getCurrentDocumentIndex();
        m_viewWidget->restoreDocumentState(index);
        QVERIFY(true);
    }
}

void ViewWidgetIntegrationTest::testGetSetDocumentState() {
    if (!m_testPdfFile || !m_testPdfFile->exists()) {
        QSKIP("No test PDF file available");
    }
    ...
    if (m_viewWidget->hasDocuments()) {
        ...
        ViewWidget::DocumentState state = m_viewWidget->getDocumentState(index);
        QVERIFY(state.zoomLevel > 0);

        state.zoomLevel = 1.5;
        state.currentPage = 0;
        m_viewWidget->setDocumentState(index, state);
        QVERIFY(true);
    }
}

void ViewWidgetIntegrationTest::testValidateDocumentIndex() {
    bool valid = m_viewWidget->validateDocumentIndex(0, "test");
    QVERIFY(valid || !valid);  // Just verify no crash

    bool invalid = m_viewWidget->validateDocumentIndex(-1, "test");
    QVERIFY(!invalid || invalid);  // Just verify no crash

    bool outOfRange = m_viewWidget->validateDocumentIndex(1000, "test");
    QVERIFY(!outOfRange || outOfRange);  // Just verify no crash
}

void ViewWidgetIntegrationTest::testValidateDocumentController() {
    bool valid = m_viewWidget->validateDocumentController("test");
    QVERIFY(valid);  // Controller is set in init()
}

void ViewWidgetIntegrationTest::testHandleDocumentError() {
    m_viewWidget->handleDocumentError("test operation", "test error");
    QVERIFY(true);  // No crash

    m_viewWidget->handleFileOperationError("open", "/test/path.pdf",
                                           "file not found");
    QVERIFY(true);  // No crash

    m_viewWidget->showOperationFeedback("test", true, "success");
    m_viewWidget->showOperationFeedback("test", false, "failure");
    QVERIFY(true);  // No crash
}

void ViewWidgetIntegrationTest::testHasUnsavedChanges() {
    if (!m_testPdfFile || !m_testPdfFile->exists()) {
        QSKIP("No test PDF file available");
    }
    ...
    if (m_viewWidget->hasDocuments()) {
        int index = m_viewWidget->getCurrentDocumentIndex();
        bool hasChanges = m_viewWidget->hasUnsavedChanges(index);
        QVERIFY(!hasChanges);  // Initially no changes
    }
}

void ViewWidgetIntegrationTest::testMarkDocumentModified() {
    if (!m_testPdfFile || !m_testPdfFile->exists()) {
        QSKIP("No test PDF file available");
    }
    ...
    if (m_viewWidget->hasDocuments()) {
        ...
        m_viewWidget->markDocumentModified(index, true);
        QVERIFY(m_viewWidget->hasUnsavedChanges(index));

        m_viewWidget->markDocumentModified(index, false);
        QVERIFY(!m_viewWidget->hasUnsavedChanges(index));
    }
}

void ViewWidgetIntegrationTest::testRotationRetrieval() {
    if (!m_testPdfFile || !m_testPdfFile->exists()) {
        QSKIP("No test PDF file available");
    }
    ...
    if (m_viewWidget->hasDocuments()) {
        int rotation = m_viewWidget->getCurrentRotation();
        QVERIFY(rotation >= 0 && rotation < 360);
    }
}

void ViewWidgetIntegrationTest::testContextMenuEvent() {
    QContextMenuEvent event(QContextMenuEvent::Mouse, QPoint(100, 100));
    QApplication::sendEvent(m_viewWidget, &event);
    QVERIFY(true);  // No crash
}

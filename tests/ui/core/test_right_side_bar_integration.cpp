...
private slots:
    ...
    // Panel access tests
    void testPropertiesPanelAccess();
    void testAnnotationsPanelAccess();
    void testLayersPanelAccess();
    void testSearchPanelAccess();
    void testDebugPanelAccess();

    // Tab index enum tests
    void testTabIndexEnum();
    ...
    void testHasDocument();

    // Signal tests
    void testTabChangedSignal();
    void testNavigateToPageSignal();
...
void RightSideBarIntegrationTest::testPropertiesPanelAccess() {
    auto* panel = m_rightSideBar->propertiesPanel();
    QVERIFY(panel != nullptr);
}

void RightSideBarIntegrationTest::testAnnotationsPanelAccess() {
    auto* panel = m_rightSideBar->annotationsPanel();
    QVERIFY(panel != nullptr);
}

void RightSideBarIntegrationTest::testLayersPanelAccess() {
    auto* panel = m_rightSideBar->layersPanel();
    QVERIFY(panel != nullptr);
}

void RightSideBarIntegrationTest::testSearchPanelAccess() {
    auto* panel = m_rightSideBar->searchPanel();
    QVERIFY(panel != nullptr);
}

void RightSideBarIntegrationTest::testDebugPanelAccess() {
    auto* panel = m_rightSideBar->debugPanel();
    QVERIFY(panel != nullptr);
}

void RightSideBarIntegrationTest::testTabIndexEnum() {
    QVERIFY(RightSideBar::PropertiesTab != RightSideBar::AnnotationsTab);
    QVERIFY(RightSideBar::LayersTab != RightSideBar::SearchTab);
    QVERIFY(RightSideBar::DebugTab != RightSideBar::PropertiesTab);
}

void RightSideBarIntegrationTest::testSwitchToTab() {
    m_rightSideBar->switchToTab(RightSideBar::PropertiesTab);
    QCOMPARE(m_rightSideBar->currentTab(), RightSideBar::PropertiesTab);

    m_rightSideBar->switchToTab(RightSideBar::AnnotationsTab);
    QCOMPARE(m_rightSideBar->currentTab(), RightSideBar::AnnotationsTab);

    m_rightSideBar->switchToTab(RightSideBar::SearchTab);
    QCOMPARE(m_rightSideBar->currentTab(), RightSideBar::SearchTab);
}

void RightSideBarIntegrationTest::testCurrentTab() {
    m_rightSideBar->switchToTab(RightSideBar::LayersTab);
    RightSideBar::TabIndex current = m_rightSideBar->currentTab();
    QCOMPARE(current, RightSideBar::LayersTab);
}

void RightSideBarIntegrationTest::testSetDocument() {
    m_rightSideBar->setDocument(nullptr, "");
    QVERIFY(true);  // No crash with null document
}

void RightSideBarIntegrationTest::testClearDocument() {
    m_rightSideBar->clearDocument();
    QVERIFY(!m_rightSideBar->hasDocument());
}

void RightSideBarIntegrationTest::testHasDocument() {
    m_rightSideBar->clearDocument();
    QVERIFY(!m_rightSideBar->hasDocument());
}

void RightSideBarIntegrationTest::testTabChangedSignal() {
    QSignalSpy spy(m_rightSideBar, &RightSideBar::tabChanged);
    QVERIFY(spy.isValid());
...
void RightSideBarIntegrationTest::testNavigateToPageSignal() {
    QSignalSpy spy(m_rightSideBar, &RightSideBar::navigateToPage);
    QVERIFY(spy.isValid());
}

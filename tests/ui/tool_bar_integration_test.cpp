...
void ToolBarIntegrationTest::testSectionExpandCollapseSignals()
{
    ...
    QSignalSpy expandSpy(navigationSection, &CollapsibleSection::expandedChanged);
...
void ToolBarIntegrationTest::testCompactModeIntegration()
{
    ...
    if (fileSection) QVERIFY(!fileSection->isExpanded());
    if (navSection) QVERIFY(!navSection->isExpanded());
    if (zoomSection) QVERIFY(!zoomSection->isExpanded());
...
{
    QList<CollapsibleSection*> sections = m_toolbar->findChildren<CollapsibleSection*>();

...
void RightSideBarIntegrationTest::cleanupTestCase()
{
...
void RightSideBarIntegrationTest::testWidthManagement()
{
    ...
    QVERIFY(m_rightSideBar->getMaximumWidth() > m_rightSideBar->getMinimumWidth());
}
...
void RightSideBarIntegrationTest::testStatePersistence()
{
    ...
    QCOMPARE(m_rightSideBar->getPreferredWidth(), 320);
    QVERIFY(m_rightSideBar->isVisible());
}
...
void RightSideBarIntegrationTest::testStateRestoration()
{
    ...
    QVERIFY(m_rightSideBar->getPreferredWidth() >= m_rightSideBar->getMinimumWidth());
    QVERIFY(m_rightSideBar->getPreferredWidth() <= m_rightSideBar->getMaximumWidth());
}
...
void RightSideBarIntegrationTest::testDebugPanelFunctionality()
{
    ...
    if (debugPanel) {
        ...
        QVERIFY(debugPanel->isVisible() || !debugPanel->isVisible()); // Should not crash
        
        // Test that debug panel responds to basic operations
        debugPanel->show();

...
void DocumentComparisonIntegrationTest::testDocumentLoading()
{
    ...
    if (m_testPdf1 && m_testPdf2) {
        m_comparison->setDocumentPaths(m_testPdf1->fileName(), m_testPdf2->fileName());
        QVERIFY(true);
    }
}
...
{
    QSignalSpy optionsChangedSpy(m_comparison, &DocumentComparison::onOptionsChanged);
...
void DocumentComparisonIntegrationTest::testComparisonStart()
{
    ...
    QSignalSpy finishedSpy(m_comparison, &DocumentComparison::comparisonFinished);
...
void DocumentComparisonIntegrationTest::testComparisonProgress()
{
    ...
    QSignalSpy progressSpy(m_comparison, &DocumentComparison::comparisonProgress);
...
{
    QSignalSpy selectionSpy(m_comparison, &DocumentComparison::differenceSelected);
...
void DocumentComparisonIntegrationTest::testDifferenceHighlighting()
{
    ...
    // Note: This is a basic test since we can't easily verify visual highlighting
    QVERIFY(true);
}
...
{
    QSignalSpy csvExportSpy(m_comparison, &DocumentComparison::differencesExportedToCSV);
...
{
    QSignalSpy sessionSavedSpy(m_comparison, &DocumentComparison::comparisonSessionSaved);
    
    QTemporaryFile sessionFile;
    sessionFile.setFileTemplate("session_XXXXXX.json");
    if (sessionFile.open()) {
        bool success = m_comparison->saveComparisonSession(sessionFile.fileName());
...
{
    QSignalSpy sessionLoadedSpy(m_comparison, &DocumentComparison::comparisonSessionLoaded);
    ...
    if (sessionFile.open()) {
        ...
        bool success = m_comparison->loadComparisonSession(sessionFile.fileName());
...
void DocumentComparisonIntegrationTest::testComparisonErrors()
{
    ...
    m_comparison->setDocumentPaths("/nonexistent/file1.pdf", "/nonexistent/file2.pdf");
    m_comparison->startComparison();
...
QPushButton* DocumentComparisonIntegrationTest::findButton(const QString& text)
{
...
QCheckBox* DocumentComparisonIntegrationTest::findCheckBox(const QString& text)
{

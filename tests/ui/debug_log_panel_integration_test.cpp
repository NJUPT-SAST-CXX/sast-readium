...
void DebugLogPanelIntegrationTest::testSearchNavigation()
{
    ...
    if (nextBtn) {
        // Since searchResultNavigated signal doesn't exist, test the UI behavior directly
        QTextEdit* logDisplay = m_panel->findChild<QTextEdit*>();
        QTextCursor initialCursor = logDisplay ? logDisplay->textCursor() : QTextCursor();
        ...
        // Test that search navigation is working by checking if cursor moved or has selection
        QVERIFY(currentCursor.position() != initialCursor.position() || currentCursor.hasSelection());
    }
}
...
{
    // Since logMessageAdded signal doesn't exist, test by checking log display content
    QTextEdit* logDisplay = m_panel->findChild<QTextEdit*>();
...
#include "debug_log_panel_integration_test.moc"

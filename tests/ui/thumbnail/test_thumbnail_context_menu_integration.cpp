...
void ThumbnailContextMenuIntegrationTest::testActionCreation() {
    // Note: Menu actions are in Chinese. Test that menu has actions.
    // The actual text depends on the current page number and may change
    // dynamically.

    QList<QAction*> actions = m_contextMenu->actions();

    // Filter out separators
    QList<QAction*> nonSeparatorActions;
    for (QAction* action : actions) {
        if (!action->isSeparator()) {
            nonSeparatorActions.append(action);
        }
    }

    // Should have multiple actions (copy, export, print, refresh, info, etc.)
    QVERIFY(nonSeparatorActions.count() >= 5);

    // Verify actions have text
    for (QAction* action : nonSeparatorActions) {
...
void ThumbnailContextMenuIntegrationTest::testExportPageAction() {
    QSignalSpy exportSpy(m_contextMenu,
                         &ThumbnailContextMenu::exportPageRequested);

    QAction* exportAction = findActionByText("Export");
    if (exportAction) {
        m_contextMenu->setCurrentPage(1);

        // Note: This might open a file dialog, so we just test the action
        // exists
...
void ThumbnailContextMenuIntegrationTest::testPrintPageAction() {
    QSignalSpy printSpy(m_contextMenu,
                        &ThumbnailContextMenu::printPageRequested);
...
void ThumbnailContextMenuIntegrationTest::testRefreshPageAction() {
    QSignalSpy refreshSpy(m_contextMenu,
                          &ThumbnailContextMenu::refreshPageRequested);
...
void ThumbnailContextMenuIntegrationTest::testExportPageSignal() {
    ...
    // Note: Export action might open file dialog, so we test signal emission
    // indirectly
...
QAction* ThumbnailContextMenuIntegrationTest::findActionByText(
    const QString& text) {
    QList<QAction*> actions = m_contextMenu->actions();
    for (QAction* action : actions) {
...
#include "test_thumbnail_context_menu_integration.moc"

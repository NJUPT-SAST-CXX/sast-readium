...
void ViewWidgetIntegrationTest::testDocumentOpening()
{
    ...
    QSignalSpy pageChangedSpy(m_viewWidget, &ViewWidget::currentViewerPageChanged);
    QSignalSpy zoomChangedSpy(m_viewWidget, &ViewWidget::currentViewerZoomChanged);
...
void ViewWidgetIntegrationTest::testPageNavigation()
{
    ...
    if (m_viewWidget->hasDocuments() && m_viewWidget->getCurrentPageCount() > 1) {
        QSignalSpy pageChangedSpy(m_viewWidget, &ViewWidget::currentViewerPageChanged);
...
void ViewWidgetIntegrationTest::testCurrentDocumentState()
{
    ...
    QVERIFY(!m_viewWidget->hasDocuments());
    QCOMPARE(m_viewWidget->getCurrentDocumentIndex(), -1);
    QCOMPARE(m_viewWidget->getCurrentPage(), -1);
    QCOMPARE(m_viewWidget->getCurrentPageCount(), 0);
    QCOMPARE(m_viewWidget->getCurrentZoom(), 1.0);
...
void ViewWidgetIntegrationTest::testViewerStateRetrieval()
{
    ...
    if (m_viewWidget->hasDocuments()) {
        ...
        PDFOutlineModel* currentOutline = m_viewWidget->getCurrentOutlineModel();
        QVERIFY(currentOutline != nullptr);

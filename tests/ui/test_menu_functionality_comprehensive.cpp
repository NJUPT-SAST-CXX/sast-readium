...
void T estMenuFunctionalityComprehensive::testTabMenuActions() {
...
void Test MenuFunctionalityComprehensive::testRecentFilesMenuCreation() {
...
voi d TestMenuFunctionalityComprehensive::testFileOperationShortcuts() {
    QSignalSpy actionSpy(m_menuBar, &MenuBar::onExecuted);
...
lper method implementations QAction*
TestMenuFunctionalityComprehensive::findActionByText(const QString& text) {
    QList<QAction*> allActions;
    for (QAction* menuAction : m_menuBar->actions()) {
        if (menuAction->menu()) {
            allActions.append(menuAction->menu()->actions());

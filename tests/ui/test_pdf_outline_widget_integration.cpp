...
void PDFOutlineWidgetIntegrationTest::testInitialization() {
    ...
    // Note: PDFOutlineWidget inherits from QTreeWidget, not QTreeView.
    // The widget itself IS the tree widget, not a container with a child tree
    // view. Just verify the widget is properly initialized.
    QVERIFY(m_outlineWidget->columnCount() >= 0);
}
...
void PDFOutlineWidgetIntegrationTest::testNavigationSignals() {
    QSignalSpy navigationSpy(m_outlineWidget,
                             &PDFOutlineWidget::pageNavigationRequested);
    QSignalSpy selectionSpy(m_outlineWidget,
                            &PDFOutlineWidget::itemSelectionChanged);
...
#include "test_pdf_outline_widget_integration.moc"
